...
const DateSelector = ({date ,setDate}) => {
    ...
    return (
        <div>
            <button className='inline-flex items-center gap-2 text-[13px] font-medium text-blue-900
                bg-sky-200/40 hover:bg-sky-200/70 rounded-sm px-2 py-1 cursor-pointer'
            ...
            {openDatePicker && (
                <div className='overflow-y-scroll p-5 bg-sky-50/80 rounded-lg relative pt-9'>
                    ...
                    <DayPicker captionLayout='dropdown' mode='single' selected={date} onSelect={setDate} pagedNavigation className='*:text-sm'/>
                </div>
            )}
        </div>
    )
